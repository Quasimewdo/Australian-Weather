...
```{r}
data <-read.csv("~/Documents/AnalyseDon/Projet/weatherAUS.csv")
head (data)
str(data)
dim(data)
```
...
Au vu du dendogramme, on va garder 4 ou 5 groupes  car après les hauteurs des branches deviennent trop grandes (à l'oeil, on de voir les paquets qui se détachent).
```{r}
cah.ward$height
...
Au début faible perte d'inertie donc le regoupement est pertinent. 
...
#Plot selon Tla Température moyenne
```{r}
...
Classe 3 : Température moyenne-moyenne, Humidité forte, Pression faible
...
Cluster 2 a des silouettes proche de 1, donc bien placés.
Cluster 1 a des individus dont silhouettes négétifs, donc mal placés.
